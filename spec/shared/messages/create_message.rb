RSpec.shared_examples 'POST /:id/messages' do
  describe 'POST /:id/messages' do
    ...
    describe 'Nominal case' do
      before do
        post "/campaigns/#{campaign.id}/messages", {token: 'test_token', app_key: appli.key, session_id: session.token, content: 'test'}
      end
    ...
    describe '400 errors' do
      describe 'message not given' do
        before do
          post "/campaigns/#{campaign.id}/messages", {token: 'test_token', app_key: appli.key, session_id: session.token}
        end
      ...
      describe 'message given empty' do
        before do
          post "/campaigns/#{campaign.id}/messages", {token: 'test_token', app_key: appli.key, session_id: session.token, content: ''}
        end
        it 'Returns a Bad Request (400) status' do
          expect(last_response.status).to be 400
        end
      ...
      describe 'session ID not given' do
        before do
          post "/campaigns/#{campaign.id}/messages", {token: 'test_token', app_key: appli.key}
        end
        it 'Returns a Bad Request (400) status' do
          expect(last_response.status).to be 400
        end
